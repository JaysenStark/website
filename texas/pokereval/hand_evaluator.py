...
class HandEvaluator:
        ...
        def evaluate_rank(hand):
            ...
            if has_flush:
                ...
                if possible_rank != 0:
                    ...
                    q = reduce(mul, list(map(lambda card: card & 0xFF, bh)))
    ...
    class Six:
        ...
        def evaluate_rank(hand):
            ...
            flush_prime = reduce(mul, list(map(lambda card: (card >> 12) & 0xF, bh)))
            flush_suit = False
            ...
            if even_xor == 0: # x-0
                ...
                if odd_popcount == 4: # 4-0
                    prime_product = reduce(mul, list(map(lambda card: card & 0xFF, bh)))
                ...
                if even_popcount == 2: # 0-2
                    prime_product = reduce(mul, lidt(map(lambda card: card & 0xFF, bh)))
                    return LookupTables.Six.prime_products_to_rank[prime_product]
                else: # 0-3
        ...
        def evaluate_rank(hand):
            ...
            if flush_suit:
                ...
                if even_xor == 0:
                    ...
                    bits = reduce(__or__, list(map(
                        lambda card: (card >> 16),
                        filter(lambda card: (card >> 12) & 0xF == flush_suit, bh)))
                    )
    ...
    def evaluate_hand(hand, board=[]):
        ...
        if len(hand) not in hand_lengths:
            raise HandLengthException("Only %s hole cards are supported" % ", ".join(list(map(str, hand_lengths))))
